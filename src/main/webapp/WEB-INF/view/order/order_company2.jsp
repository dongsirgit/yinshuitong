        ...
        <table class="ordertable ac">
        	<tr>
            	...
            	<td class="ordertable_r">
                	<div>
                    	...
                    	<c:if test="${!empty attachMap['2'].batchName }">
		                    <a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/showPic?id=${attachMap['2'].id }" target="_blank">${attachMap['2'].batchName }</a>
                    	</c:if>
        	...
        	<tr>
            	...
            	<td class="al ordertable_main">
                    ...
                    <p class="pl20 colorred pb20">* 下载word文档模版，填写后上传。<a href="/files/help/com_apply.docx"  target="_blank" rel="external" >下载模版>></a></p>
                </td>
            	<td class="ordertable_r">
                	<div>
        	...
        	<tr>
            	...
            	<td class="ordertable_r borbdot">
                	<div>
	                    ...
	                    <a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/showPic?id=${attachMap['4'].id }" target="_blank">${attachMap['4'].batchName }</a>
                    	</c:if>
                        </div>
                    </div>
                </td>
            	...
            	<td class="ordertable_r">
                	<div>
                        ...
                        <div id="filelist4">
	                    	...
	                    	<a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/showPic?id=${attachMap['5'].id }" target="_blank">${attachMap['5'].batchName }</a>
	                    	</c:if>
                        </div>
                    </div>
                </td>
...
<form id="returnpage1" action="<%=basePath %>/users/applyLoan/com1" method="post">
	<input type="hidden" id="qyjyid" name="id">
	<input type="hidden" id="grxftype" name="type" value='1'>
</form>
...
$(function(){
	            ...
	            if (data.status == "0") {
	               alert("请登陆...");
	            	...
	            	if(data.success=="1"){
	            		...
	            		window.parent.location.href='<%=basePath %>/users/applyLoan/com3?type=1';
	            	}else{
	            		alert("提交失败,请重新提交!");
	            	}
	            }
    ...
    var uploader = new plupload.Uploader({
        ...
        url : '<%=basePath%>/users/fileUpload/upload?pageFlag=com2&zipFlag=isZip&upfiletype=2&applyId='+id, 
...
//     	chunk_size:'2mb',
    ...
    uploader.bind('UploadComplete',function(uploader,files){
    	plupload.each(files, function(file) {
	    	document.getElementById('filelist1').innerHTML = '<a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/showPicByIdType?id='+id+'&attType=2 " target="_blank">'+file.name+'</a>';
		});
    	$("#browse1").attr('disabled',false);
    });
  ...
  //文档类上传
    var uploader_doc = new plupload.Uploader({
        ...
        url : '<%=basePath%>/users/fileUpload/upload?pageFlag=com2&zipFlag=docf&upfiletype=3&applyId='+id, 
        flash_swf_url : '<%=basePath%>/plupload/Moxie.swf', 
        silverlight_xap_url : '<%=basePath%>/plupload/Moxie.xap', 
    	filters: {
   		  mime_types : [ 
...
//	    	alert('上传完成！！！');
    	plupload.each(files, function(file) {
	    	document.getElementById('filelist2').innerHTML = '<a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/downloadByIdType?id='+id+'&attType=3 " target="_blank">'+file.name+'</a>';
		});
    	$("#browse2").attr('disabled',false);
    });
  ...
  //发票类进项上传
    var uploader_invoices_in = new plupload.Uploader({
        ...
        url : '<%=basePath%>/users/fileUpload/upload?pageFlag=com2&zipFlag=isZip&upfiletype=4&applyId='+id, 
        flash_swf_url : '<%=basePath%>/plupload/Moxie.swf', 
        silverlight_xap_url : '<%=basePath%>/plupload/Moxie.xap', 
    	filters: {
   		  mime_types : [ 
...
//	    	alert('上传完成！！！');
    	plupload.each(files, function(file) {
	    	document.getElementById('filelist3').innerHTML = '<a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/showPicByIdType?id='+id+'&attType=4 " target="_blank">'+file.name+'</a>';
		});
    	$("#browse3").attr('disabled',false);
    });
  ...
  //发票类销项上传
    var uploader_invoices_out = new plupload.Uploader({
        ...
        url : '<%=basePath%>/users/fileUpload/upload?pageFlag=com2&zipFlag=isZip&upfiletype=5&applyId='+id, 
        flash_swf_url : '<%=basePath%>/plupload/Moxie.swf', 
        silverlight_xap_url : '<%=basePath%>/plupload/Moxie.xap', 
    	filters: {
   		  mime_types : [ 
...
//     	chunk_size:'2mb',
    ...
    uploader_invoices_out.bind('UploadComplete',function(uploader_invoices_out,files){
    	plupload.each(files, function(file) {
	    	document.getElementById('filelist4').innerHTML = '<a class="pt5 tdul" name="flge4check" href="<%=basePath %>/users/file/showPicByIdType?id='+id+'&attType=5 " target="_blank">'+file.name+'</a>';
		});
    	$("#browse4").attr('disabled',false);
    });
