...
public static String generatePDF(Long uid,TaxReportWithBLOBs report) throws Exception {
...
//		PdfWriter writer = PdfWriter.getInstance(document, new FileOutputStream("d:\\征信报告.pdf"));
		...
		Paragraph para1 = new Paragraph(
				"1.1 基本信息\n"
				+ "企业名称："+report.getGsmc()+"\n"
				+ "法定代表人："+report.getQyfr()+"\n"
				+ "注册资金："+report.getZczb()+"万元\n"
				+ "成立日期："+report.getClrq()+"\n"
				+ "通信地址："+report.getTxdz()+"\n"
				+ "联系电话："+report.getLxdh()+"\n"
				+ "电子邮箱：\n"
				+ "注册号/统一社会信用代码："+report.getTyshxydm()+"\n"
				+ "税务登记代码："+report.getSwdjh()+"\n"
				+ "组织机构代码："+report.getZzjgdm()+"\n"
				+ "经营状态："+report.getJyzk()+"\n"
				+ "企业类型："+report.getQylx()+"\n"
				+ "经营范围："+report.getJyfw()+"\n"
				+ "住所："+report.getTxdz()+"\n"
				+ "登记机关："+report.getDjjg()+"\n"
				+ "营业时间起："+report.getScjysjQ()+"\n"
				+ "营业时间止："+report.getScjysjZ()+"\n"
				+ "1.2 股东信息\n"
				+ "股东姓名："+report.getGsmc()+"\n"
				+ "股东类型："+report.getGsmc()+"\n"
				+ "身份证（营业代码）："+report.getGsmc()+"\n"
				+ "认缴出资额："+report.getGsmc()+"\n"
				+ "认缴时间："+report.getGsmc()+"\n"
				+ "实缴出资额："+report.getGsmc()+"\n"
				+ "实缴时间："+report.getGsmc()+"\n"
				+ "1.3 主要成员\n"
				+ "姓名："+report.getGsmc()+"\n"
				+ "职务："+report.getGsmc()+"\n"
		...
		Paragraph para2 = new Paragraph(
				...
				+ "文号："+report.getGsmc()+"\n"
				+ "立案时间："+report.getGsmc()+"\n"
				+ "失效文书确定的义务："+report.getGsmc()+"\n"
				+ "履行情况："+report.getGsmc()+"\n"
				+ "2.2 劳动仲裁记录："+report.getGsmc()+"\n"
				+ "文号："+report.getGsmc()+"\n"
				+ "立案时间："+report.getGsmc()+"\n"
				+ "仲裁确定的义务："+report.getGsmc()+"\n"
				+ "履行情况："+report.getGsmc()+"\n"
				+ "2.3 法院判决记录\n"
				+ "文号："+report.getGsmc()+"\n"
				+ "立案时间："+report.getGsmc()+"\n"
				+ "判决确定的义务："+report.getGsmc()+"\n"
				+ "履行情况："+report.getGsmc()+"\n"
				,new Font(bf, 8, Font.NORMAL));
		Paragraph para3 = new Paragraph(
				"3.1 增值税\n"
				+ "上上年增值税税额："+report.getGsmc()+"\n"
				+ "上年增值税税额："+report.getGsmc()+"\n"
				+ "上年比上上年增长情况："+report.getGsmc()+"\n"
				+ "本年度增值税税额："+report.getGsmc()+"\n"
				+ "一月："+report.getGsmc()+"\n"
				+ "二月："+report.getGsmc()+"\n"
				+ "....\n"
				+ "3.2营业税\n"
				+ "上上年营业税税额："+report.getGsmc()+"\n"
				+ "上年营业税税额："+report.getGsmc()+"\n"
				+ "上年比上上年增长情况："+report.getGsmc()+"\n"
				+ "本年度营业税税额："+report.getGsmc()+"\n"
				+ "一月："+report.getGsmc()+"\n"
				+ "二月："+report.getGsmc()+"\n"
				+ "....\n"
				+ "3.3 所得税\n"
				+ "上上年所得税税额："+report.getGsmc()+"\n"
				+ "上年所得税税额："+report.getGsmc()+"\n"
				+ "上年比上上年增长情况："+report.getGsmc()+"\n"
				+ "本年度所得税税额："+report.getGsmc()+"\n"
				+ "一月："+report.getGsmc()+"\n"
				+ "二月："+report.getGsmc()+"\n"
				+ "....\n"
				+ "3.4 企业资产\n"
				+ "上上年资产额："+report.getGsmc()+"\n"
				+ "上年资产额："+report.getGsmc()+"\n"
				+ "上年比上上年增长情况："+report.getGsmc()+"\n"
				+ "本年度资产额："+report.getGsmc()+"\n"
				+ "一月："+report.getGsmc()+"\n"
				+ "二月："+report.getGsmc()+"\n"
				+ "....\n"
				+ "3.4 企业利润\n"
				+ "上上年利润额："+report.getGsmc()+"\n"
				+ "上年利润额："+report.getGsmc()+"\n"
				+ "上年比上上年增长情况："+report.getGsmc()+"\n"
				+ "本年度利润额："+report.getGsmc()+"\n"
				+ "一月："+report.getGsmc()+"\n"
				+ "二月："+report.getGsmc()+"\n"
				+ "....\n"
				+ "3.4 企业流动资金余额\n"
				+ "上上年资金余额："+report.getGsmc()+"\n"
				+ "上年资金余额："+report.getGsmc()+"\n"
				+ "本年度资金余额："+report.getGsmc()+"\n"
				+ "一月："+report.getGsmc()+"\n"
				+ "二月："+report.getGsmc()+"\n"
				+ "....\n"
				,new Font(bf, 8, Font.NORMAL));
	...
	public static void main(String[] args) {
		Object obj = JSON.parse("{\"gdxm\":\"陈杰\",\"dze\":\"9000万\",\"dzsj\":\"2027年5月4日\"}");
