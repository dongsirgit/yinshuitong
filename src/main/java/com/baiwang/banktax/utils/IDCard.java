 ...
 * 
  * @ClassName: IDCard
  * @Description: 严格验证身份证Java类，支持15位与18位验证,调用.vertify()即可
  * @author liuJinGui
  * @date 2015年9月28日 下午3:37:01
 */
public class IDCard {
...
//            }
  	...
  	/**
  	 * 根据身份证号获得男女   0：男  1：女 2:未知
  	 * 倒数第二位是奇数为男性，偶数为女性，满意选我！
  	 * @param cardid
  	 * @return
  	 */
  	public static  String getSexByCardId(String cardid){
  		if(isNotBlank(cardid)){
  			String tmp_str = cardid.substring(cardid.length() -2,cardid.length()-1);
  			if(cardid.length() == 15 || cardid.length() == 18){
  				if("x".equalsIgnoreCase(tmp_str) || "e".equalsIgnoreCase(tmp_str)){
  					return "男";
  				}else{
  					 int sex = Integer.parseInt(tmp_str) % 2;
  	                return sex == 0 ? "女" : "男";
  				}
  			}
  		}
  	...
  	public static  String getBirthdayByCardId(String cardid){
  		if(isNotBlank(cardid) && (cardid.length() == 15 || cardid.length() == 18)){
  			return cardid.substring(6, 10) + "年" + cardid.substring(10, 12) + "月" + cardid.substring(12, 14)+"日"; 
  		}
  		return "";
  	}
